<div class="table-responsive">
    <table class="table invoice-detail-table">
        ...
        <tbody>
            @forelse ($service->estimate_items as $key => $item)
                ...
                <th>
                    ...
                    <div class="row" style="width: 52%;float: right;">
                        <div class="col-6 pl-1">
                            {!! Form::select('discount_type', ['$' => '$', '%' => '%'], $service->estimate_discount_type, [
                                'class' => 'form-control discount_type',
                            ]) !!}
                        </div>
                        <div class="col-6 p-0">
                            {!! Form::number('discount', $service->estimate_discount, [
...
<div class="row">
    <div class="col-sm-6">
        ...
        {!! Form::textarea('term', $service->estimate_terms, ['class' => 'form-control', 'rows' => 2]) !!}
    </div>
    <div class="col-sm-6">
        <h6>Estimate notes visible to client:</h6>
        {!! Form::textarea('note', $service->estimate_note, ['class' => 'form-control', 'rows' => 2]) !!}
    </div>
</div>
...
@push('scripts')
    ...
    <script type="text/javascript">
        var i = {{ count($service->estimate_items) }};
        var items = [];

        @forelse ($service->estimate_items as $item)
        ...
        $('document').ready(function() {
            ...
            $(document).on("submit", "#formValidation", function(e) {
                ...
                $.ajax({
                    type: 'post',
                    url: route('services.updateEstimate'),
                    processData: false,
                    data: formData,
